...
def fit_weibull(x):
    def optfun(theta):
        return -np.sum(
            np.log(exponweib.pdf(x, 1, theta[0], scale=theta[1], loc=0)))
...
def test_rndweibull():
    ...
    for i, shape in enumerate(tqdm(shapes)):
        ...
        for j, scale in enumerate(scales):
            ...
            if np.isnan(mle_shape):
                print('Shape is nan for shape=%5.3f, scale=%5.3f' % (
                    shape, scale))
                continue
            if np.isnan(mle_scale):
                print('Scale is nan for shape=%5.3f, scale=%5.3f' % (
                    shape, scale))
                continue
    ...
    print('RMSE shape: %7.5f, RMSE scale: %7.5f' % (
        np.sqrt(np.mean(errs[0] ** 2)), np.sqrt(np.mean(errs[1] ** 2))))
    print('Max Err shape: %7.5f, Max Err scale: %7.5f' % (
        np.max(np.abs(errs[0])), np.max(np.abs(errs[1]))))
