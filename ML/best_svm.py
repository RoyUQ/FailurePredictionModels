...
def child_optimize(train_data, test_data, my_type):
    ...
    svm_model.train_y = y
    # 参数优化
    best_params = svm_model.optimize()
    # 建立最优参数对应的模型
    ...
    # '发动机转速方差', '油泵转速方差', '泵送压力方差', '液压油温方差', '流量档位方差',
    # '分配压力方差', '排量电流方差', '低压开关', '高压开关', '搅拌超压信号', '正泵', '反泵'
    test_y = np.array(test_data['标签'])
    my_svm.fit(x, y)
    # 储存模型
    with open(default_path + "svm_" + my_type, 'wb') as f:
...
if __name__ == '__main__':
    ...
    #     print('----------------------' + my_type + '----------------------')
    #     child_optimize(train_data, test_data, my_type)
    # types = ['ZV7e8e3']
    ...
    for my_type in types:  # 对所有类型逐个进行参数优化并预测
        ...
        final_result = pd.concat([final_result, result], axis=0,
                                 ignore_index=True)
    # 将整合的预测结果输出成csv文件
    final_result.to_csv(default_path + 'svm_result.csv')
    # 读取测试数据的结果
